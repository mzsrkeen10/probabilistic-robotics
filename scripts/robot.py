...
if __name__ == "__main__":
    ...
    # for i in range(1):
    #     r = Robot(np.array([0, 0, 0]).T, sensor=None, agent=circling, color="gray", noise_per_meter=0, bias_rate_stds=(0., 0.), expected_stuck_time=60.0, expected_escape_time=60.0)
    #     world.append(r)
    # for i in range(1):
    #     r = Robot(np.array([0, 0, 0]).T, sensor=None, agent=circling, color="gray", noise_per_meter=0, bias_rate_stds=(0., 0.), expected_kidnap_time=5)
    #     world.append(r)
    # nobias_robot = IdealRobot(np.array([0, 0, 0]).T, sensor=None, agent=circling, color="red")
    # world.append(nobias_robot)
    # biased_robot = Robot(np.array([0, 0, 0]).T, sensor=None, agent=circling, color="blue", noise_per_meter=0, bias_rate_stds=(0.2, 0.2))
    # world.append(biased_robot)
    
    r = Robot(np.array([0, 0, 0]).T, sensor=Camera(m, phantom_prob=0.5, oversight_prob=0.5, occlusion_prob=0.5), agent=circling)
    world.append(r)
